    ...
    def forward(self, x):
        ...
        for i in range(len(feature_h)):
            if i==0:
                ...
                sources.append(self.relu(self.bn2(self.convs(torch.cat(temp, 1)))))
...
bottle = {'512': [[0, 1, 2, 3], [768, 768, 768, 768], [256], [1, 1, 3, 5]]}
mbox = {'512': [4, 6, 6, 4, 256]# number of boxes per feature map location
}

def vgg(infos, i,vgg_dilation, batch_norm=False):
    layers = []
    in_channels = i
        ...
        for v in cfg:
            if v == 'M':
                ...
                if name == 'conv5':
                    ...
                    ker = 3
                    pad = vgg_dilation[j]
                    conv2d = nn.Conv2d(in_channels,v,kernel_size=3,stride=1, padding=pad, dilation=rate)
                    j+=1
                else:
    ...
    conv6 = nn.Conv2d(512, 768, kernel_size=3, padding=1, dilation=1)
...
def bottleneck(cfg):
    ...
    for i in range(len(cfg[0])):
        layers+= [nn.Conv2d(cfg[1][i], cfg[2][0], kernel_size=3, stride=1, padding=cfg[3][i], dilation=cfg[3][i])]
    return layers
...
def get_pfp_net(mode, size, classes):
    layers = [0,1,2]
    backbone_, spp_, bottleneck_ , head_= multibox(vgg(base[str(size)],3,vgg_dilation), spp(extra_spp[str(size)]),
