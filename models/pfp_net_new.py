...
class PFPNet(nn.Module):
    ...
    def __init__(self, layers, status, size, classes, backbone_, spp_, bottleneck_):
...
def multibox(vgg,spp,bottleneck):

    return vgg, spp, bottleneck

def get_pfp_net(mode, size, classes):
    layers = [0,1,2]
    backbone_, spp_, bottleneck_ = multibox(vgg(base[str(size)],3), spp(extra_spp[str(size)]),
                                            bottleneck(bottle[str(size)]))

    model = PFPNet(layers, mode, size, classes, backbone_, spp_, bottleneck_)
    return model
